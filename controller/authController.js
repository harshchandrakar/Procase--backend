...
exports.userLogin = catchAsyncError(async (req, res) => {
  ...
  if (!email || !password) {
    res
      .status(400)
      .json({ success: false, meassage: "please fill input field" });
    return;
  }
  ...
  if (!user) {
    res
      .status(404)
      .json({ success: false, message: "User doesn't exists please register" });
    return;
  }
...
exports.forgotPassWord = catchAsyncError(async (req, res) => {
  ...
  sendEmail(
    email,
    "Pin for changing password",
    `Your pin for changing password is ${pin}`
  );
...
exports.updatePassword = catchAsyncError(async (req, res) => {
  ...
  if (!email) {
    res
      .status(401)
      .json({ success: false, message: "please fill email field" });
    return;
  }
  password = await bycrypt.hash(password, 10);
